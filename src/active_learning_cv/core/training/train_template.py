...
class Active_Learning_Train:
    ...
    def _create_aml_label_dataset(self, datastore, target_path, input_ds, dataset_name, prefix, register=True, simulation=False):
        ...
        return dataset
    def train_validation_split(self,ws,datastore, ds_prefix,target_path, simulation=False, random_state=101):
        train_dataset, val_dataset, current_dataset_name =self._retrieve_dataset(ws,ds_prefix, test_size=0.2, simulation=simulation,random_state=random_state)

...
| summarize value_list= make_list({numerical_column}), count_list = make_list(['count']) by ['{time_stamp_col}']
    ...
    def launch_dashboard(self):
        ...
        @app.callback(
            ...
            Input('tables', 'value'))
        def set_columns_options(table_name):
            return self.list_table_columns(table_name)['AttributeName'].values
        @app.callback(
            Output('column-name-type', 'data'),
            Input('tables', 'value'))
        def set_columns_options(table_name):
            return self.list_table_columns(table_name).to_json(date_format='iso', orient='split')
        ...
        def calculate(table_name, base_start_date, base_end_date,target_table_name,target_start_date, target_end_date,n_clicks):
            ...
            query_df = self.analyze_drift(limit=100000,base_table_name = f'{table_name}', target_table_name=f'{target_table_name}', base_dt_from=f'{base_start_date}', base_dt_to=f'{base_end_date}', target_dt_from=f'{target_start_date}', target_dt_to=f'{target_end_date}', bin='30d')
        ...
        def drilldown(click_data,table, column):
            ...
            if trigger_id == 'graph':
                ...
                if click_data is not None:
                    ...
                    output = self.sample_data_points(table,column,start_date,horizon=30)
